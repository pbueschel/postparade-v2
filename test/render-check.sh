...
FAIL=0
# A private profile per run: successive headless launches sharing the default
# user-data-dir contend on its lock and eventually hang mid-suite.
PROFILE="${TMPDIR:-/tmp}/pp2-render-check.$$"
...
check() {
  ...
  dom=$("$CHROME" --headless=new --disable-gpu --no-first-run --user-data-dir="$PROFILE" \
    --virtual-time-budget=6000 --dump-dom "file://$DIR/app.html$route" 2>/dev/null \
...
dom=$("$CHROME" --headless=new --disable-gpu --no-first-run --user-data-dir="$PROFILE" \
  --virtual-time-budget=4000 --dump-dom "file://$DIR/index.html" 2>/dev/null)
